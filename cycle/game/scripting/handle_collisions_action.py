...
class HandleCollisionsAction(Action):
    ...
    def _handle_segment_collision(self, cast):
        ...
        for snake in snakes:
            head = snake.get_segments()[0]
            segments = snake.get_segments()[1:]
            self._is_game_over = self._is_segment_collapsing_segments(head, segments)
            if self._is_game_over:
                break
        if not self._is_game_over:
            snakeSegments1 = snakes[0].get_segments()
            snakeSegments2 = snakes[1].get_segments()
            ...
            if self._is_game_over:
                for segment in snakeSegments2:
                    self._is_game_over = self._is_game_over or self._is_segment_collapsing_segments(segment, snakeSegments1)


    def _is_segment_collapsing_segments(self, segment, segments):
        for s in segments:
            if segment.get_position().equals(s.get_position()):
                return True
